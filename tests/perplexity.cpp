...
static std::string read_text(std::string path) {
    ...
    QWEN_CHECK(fin) << "cannot open file " << path;
...
static float cross_entropy(const ggml_tensor *input, const ggml_tensor *target) {
    QWEN_CHECK(ggml_is_contiguous(input) && ggml_n_dims(input) == 2 && input->type == GGML_TYPE_F32);
    QWEN_CHECK(ggml_is_contiguous(target) && ggml_n_dims(target) == 1 && target->type == GGML_TYPE_I32);
    QWEN_CHECK(input->ne[1] == target->ne[0]);

    const int num_classes = input->ne[0]; //vocab size
    const int batch_size = input->ne[1]; // seq len
...
static void perplexity(Args &args) {
    ...
    for (size_t begin = 0; begin < corpus_ids.size(); begin += args.stride) {
        ...
        auto ctx = qwen::make_unique_ggml_context(1024 * qwen::MB, nullptr, false);
