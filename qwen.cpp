...
auto ModelLoader::read_tensor(const std::string &name, ggml_tensor *tensor) -> void {
    ...
    // read and check tensor shape
    {
        ...
        if ((n_dims == 1) && (ndim == 2) && (tensor->ne[1] == 1))
            n_dims = 2;
        QWEN_CHECK(ndim == n_dims) << "tensor " << name << " ndim mismatch: expect " << n_dims << " but got " << ndim;
        for (int i = ndim - 1; i >= 0; i--) {
            int dim_size = read_basic<int>();
            QWEN_CHECK(dim_size == tensor->ne[i]) << "tensor " << name << " shape mismatch at dim " << i << ": expect "
                                                  << tensor->ne[i] << " but got " << dim_size;
        }
    }
...
LlamaTokenizer::LlamaTokenizer(const std::string &tiktoken_path, const QwenConfig &config)
    : QwenTokenizer(tiktoken_path, config) {
    std::ifstream file(tiktoken_path);
    if (!file) {
        throw std::runtime_error("failed to open encoder file: " + tiktoken_path);
    }
...
std::string LlamaTokenizer::build_prompt(const std::vector<ChatMessage> &messages) {
    ...
    // 帮我介绍一下上海迪士尼的游玩攻略<|eot_id|><|start_header_id|>assistant<|end_header_id|>

    oss_prompt << "<|begin_of_text|><|start_header_id|>system<|end_header_id|>>\n\n"
               << messages.front().content << "<|eot_id|>";

    for (size_t i = 1; i < messages.size(); i += 1) {     // get all history messages
        if (messages[i].role == ChatMessage::ROLE_USER) { // user
            oss_prompt << "<|start_header_id|>user<|end_header_id|>\n\n"
                       << messages[i].content << "<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n";
        } else { // assitant
            oss_prompt << messages[i].content << "";
        }
    }
    // std::cout << oss_prompt.str()<<std::endl; //debug
...
std::vector<int> LlamaTokenizer::encode_messages(const std::vector<ChatMessage> &messages, int max_length) const {
    std::string prompt = build_prompt(messages);
    std::vector<int> input_ids = encode(prompt, max_length);
    return input_ids;
}
...
QwenAttention::QwenAttention(ModelContext *ctx, int hidden_size, int num_attention_heads, int num_kv_heads,
                             int max_length)
    : num_attention_heads(num_attention_heads), num_kv_heads(num_kv_heads),
      q_proj(ctx, hidden_size, hidden_size, true), // head_dim * num_attention_heads
      k_proj(ctx, hidden_size, hidden_size / (num_attention_heads / num_kv_heads),
             true), // head_dim * num_kv_heads. Qwen1.5 32B is GQA model
      v_proj(ctx, hidden_size, hidden_size / (num_attention_heads / num_kv_heads), true), // head_dim * num_kv_heads
...
LlamaAttention::LlamaAttention(ModelContext *ctx, int hidden_size, int num_attention_heads, int num_kv_heads,
                               int max_length)
    : num_attention_heads(num_attention_heads), num_kv_heads(num_kv_heads),
      q_proj(ctx, hidden_size, hidden_size, false), // head_dim * num_attention_heads
      k_proj(ctx, hidden_size, hidden_size / (num_attention_heads / num_kv_heads),
             false), // head_dim * num_kv_heads. Qwen1.5 32B is GQA model
      v_proj(ctx, hidden_size, hidden_size / (num_attention_heads / num_kv_heads), false), // head_dim * num_kv_heads
      o_proj(ctx, hidden_size, hidden_size, false),
      k_cache(ggml_new_tensor_3d(ctx->ctx_kv.get(), GGML_TYPE_F16, hidden_size / num_attention_heads, max_length,
                                 num_kv_heads)),
      v_cache(ggml_new_tensor_3d(ctx->ctx_kv.get(), GGML_TYPE_F16, max_length, hidden_size / num_attention_heads,
...
auto QwenAttention::forward(ModelContext *ctx, ggml_tensor *hidden_states, ggml_tensor *KQ_pos, int n_past,
    ...
    ggml_tensor *query_layer =
        ggml_view_3d(gctx, q, head_size, num_attention_heads, qlen, head_size * ggml_element_size(q), q->nb[1], 0);

    ggml_tensor *k = k_proj.forward(ctx, hidden_states); // [qlen, kv_hidden]
    // [qlen, kv_heads, head_size]
    ggml_tensor *key_layer =
        ggml_view_3d(gctx, k, head_size, num_kv_heads, qlen, head_size * ggml_element_size(k), k->nb[1], 0);

    ggml_tensor *v = v_proj.forward(ctx, hidden_states); // [qlen, kv_hidden]
    ggml_tensor *value_layer =
        ggml_view_3d(gctx, v, head_size, num_kv_heads, qlen, head_size * ggml_element_size(v), v->nb[1], 0);

    // std::cout << "attn debug 1: qkv forward pass" << std::endl;
#ifdef GGML_USE_CUBLAS
    if (!ggml_is_contiguous(query_layer)) {
        query_layer = tensor_assign_buffers(ggml_cont(gctx, query_layer));
    }
#endif
    query_layer = tensor_assign_buffers(ggml_rope_custom_inplace(gctx, query_layer, KQ_pos, rope_dim, 2, n_ctx, 0,
                                                                 rope_theta, 1.f, 0.0f, 1.0f, 0.0f, 0.0f));
...
#endif
    key_layer = tensor_assign_buffers(ggml_rope_custom_inplace(gctx, key_layer, KQ_pos, rope_dim, 2, n_ctx, 0,
                                                               rope_theta, 1.f, 0.0f, 1.0f, 0.0f, 0.0f));
    ...
    attn_scores = tensor_assign_buffers(ggml_scale_inplace(gctx, attn_scores, 1.f / std::sqrt(head_size)));
    if (n_past == 0) {
...
auto LlamaAttention::forward(ModelContext *ctx, ggml_tensor *hidden_states, ggml_tensor *KQ_pos, int n_past,
                             int n_ctx) const -> ggml_tensor * {
...
auto Qwen2MoeSparseMoeBlock::forward(ModelContext *ctx, ggml_tensor *hidden_states, int num_experts,
    ...
    ggml_tensor *weights = ggml_get_rows(gctx, ggml_reshape_3d(gctx, probs, 1, n_expert, n_tokens), selected_experts);
    ...
    for (int i = 0; i < num_experts_per_tok; i++) {
        ...
        // 最新版的ggml为了适配qwenmoe这种模型，修改了这个ggml_mul_mat_id， 这部分应该要做多的修改才能适配
        // https://github.com/ggerganov/llama.cpp/pull/6074/commits/56a38c46f5343596f69eed9803f3ec06010aecea#diff-150dc86746a90bad4fc2c3334aeb9b5887b3adad3cc1459446717638605348ef
        // https://github.com/ggerganov/llama.cpp/commit/f4dea7da1841a92d2788b0535063abf2f0e28461#diff-150dc86746a90bad4fc2c3334aeb9b5887b3adad3cc1459446717638605348ef
        ggml_tensor *cur_up = ggml_mul_mat_id(gctx, expert_ups.data(), n_expert, selected_experts, i, hidden_states);
        ggml_tensor *cur_gate =
            ggml_mul_mat_id(gctx, expert_gates.data(), n_expert, selected_experts, i, hidden_states);

        cur_gate = ggml_silu(gctx, cur_gate);
        cur_expert = ggml_mul(gctx, cur_up, cur_gate); // [n_tokens, n_embd]
        cur_expert =
            ggml_mul_mat_id(gctx, expert_downs.data(), n_expert, selected_experts, i, cur_expert); // [n_tokens, n_embd]

        cur_expert =
            ggml_mul(gctx, cur_expert, ggml_view_2d(gctx, weights, 1, n_tokens, weights->nb[1], i * weights->nb[0]));
        if (i == 0) {
            moe_out = cur_expert;
        } else {
...
auto QwenBlock::forward(ModelContext *ctx, ggml_tensor *hidden_states, ggml_tensor *KQ_pos, int n_past, int n_ctx) const
    -> ggml_tensor * {
    ggml_context *gctx = ctx->ctx_b.get();
...
auto QwenMoeBlock::forward(ModelContext *ctx, ggml_tensor *hidden_states, ggml_tensor *KQ_pos, int n_past, int n_ctx,
...
auto LlamaBlock::forward(ModelContext *ctx, ggml_tensor *hidden_states, ggml_tensor *KQ_pos, int n_past,
                         int n_ctx) const -> ggml_tensor * {
    // std::cout <<  "llamablock forward" << std::endl;
    ggml_context *gctx = ctx->ctx_b.get();
...
auto QwenModel::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx) const -> ggml_tensor * {
    ...
    // std::cout << "forward model one times"<<std::endl;
    return hidden_states;
}
...
QwenMoeModel::QwenMoeModel(ModelContext *ctx, const QwenMoeConfig &config)
    ...
    for (int layer_id = 0; layer_id < config.num_hidden_layers; layer_id++) {
        layers.emplace_back(ctx, config.hidden_size, config.num_attention_heads, config.num_kv_heads,
                            ...
                            config.shared_expert_intermediate_size, config.num_experts, config.num_experts_per_tok,
                            config.max_length);
    }
}

LlamaModel::LlamaModel(ModelContext *ctx, const Llama3Config &config)
    : embed_tokens(ctx, config.vocab_size, config.hidden_size), norm(ctx, config.hidden_size) {
    layers.reserve(config.num_hidden_layers);
    for (int layer_id = 0; layer_id < config.num_hidden_layers; layer_id++) {
        layers.emplace_back(ctx, config.hidden_size, config.num_attention_heads, config.num_kv_heads,
...
auto QwenMoeModel::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx, int num_experts,
                           int num_experts_per_tok) const -> ggml_tensor * {
    ggml_context *gctx = ctx->ctx_b.get();
    ggml_tensor *KQ_pos = pos_ids_gen_(gctx, input_ids->ne[0], n_past, n_ctx);
    if (KQ_pos) {
...
auto LlamaModel::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx) const -> ggml_tensor * {
    ggml_context *gctx = ctx->ctx_b.get();
    ggml_tensor *KQ_pos = pos_ids_gen_(gctx, input_ids->ne[0], n_past, n_ctx);
    if (KQ_pos) {
        tensor_to_device(KQ_pos);
...
QwenForCausalLM::QwenForCausalLM(const QwenConfig &config) : config(config) {
...
QwenMoeForCausalLM::QwenMoeForCausalLM(const QwenMoeConfig &config) : QwenForCausalLM(config), config(config) {
    ...
    for (int i = 0; i < config.num_hidden_layers; i++) {
        ...
        for (int j = 0; j < config.num_experts; j++) {
            ...
            state_dict_.emplace_back(prefix + "gate_proj.weight",
                                     transformer.layers[i].mlp.experts[j].gate_proj.weight);
            state_dict_.emplace_back(prefix + "up_proj.weight", transformer.layers[i].mlp.experts[j].up_proj.weight);
            state_dict_.emplace_back(prefix + "down_proj.weight",
                                     transformer.layers[i].mlp.experts[j].down_proj.weight);
        }
        state_dict_.emplace_back(layer_prefix + "mlp.shared_expert.gate_proj.weight",
                                 transformer.layers[i].mlp.shared_expert.gate_proj.weight);
        state_dict_.emplace_back(layer_prefix + "mlp.shared_expert.up_proj.weight",
                                 transformer.layers[i].mlp.shared_expert.up_proj.weight);
        state_dict_.emplace_back(layer_prefix + "mlp.shared_expert.down_proj.weight",
                                 transformer.layers[i].mlp.shared_expert.down_proj.weight);
        state_dict_.emplace_back(layer_prefix + "mlp.shared_expert_gate.weight",
                                 transformer.layers[i].mlp.shared_expert_gate.weight);
    }
    state_dict_.emplace_back("model.norm.weight", transformer.norm.weight);
    state_dict_.emplace_back("lm_head.weight", lm_head.weight);
}

Llama3ForCausalLM::Llama3ForCausalLM(const Llama3Config &config) : QwenForCausalLM(config), config(config) {
...
#endif
    constexpr size_t tensor_ovhd = GGML_TENSOR_SIZE + GGML_OBJECT_SIZE;
    const size_t ctx_w_size = (3 + config.num_hidden_layers * 9) * tensor_ovhd;
    const size_t ctx_kv_size = 2 * config.num_hidden_layers *
                               (config.max_length * config.hidden_size / config.num_attention_heads *
                                    config.num_kv_heads * ggml_type_size(GGML_TYPE_F16) +
                                tensor_ovhd);
    ...
    // build state_dict
    state_dict_.reserve(3 + config.num_hidden_layers * 9);
    state_dict_.emplace_back("model.embed_tokens.weight", transformer.embed_tokens.weight);
    for (int i = 0; i < config.num_hidden_layers; i++) {
        std::string layer_prefix = "model.layers." + std::to_string(i) + '.';
        state_dict_.emplace_back(layer_prefix + "input_layernorm.weight", transformer.layers[i].input_layernorm.weight);
...
auto QwenForCausalLM::forward_graph_compute(const std::vector<int> &input_ids, int n_past, int n_ctx, int n_threads,
                                            bool is_decoding) -> ggml_tensor * {

    ctx_.ctx_b = make_unique_ggml_context(ctx_.compute_buffer.size(), ctx_.compute_buffer.data(), false);
    // ctx_.gf = ggml_new_graph(ctx_.ctx_b.get()); // dafault graph size didn't fit larger model
...
auto QwenForCausalLM::generate_next_token(const std::vector<int32_t> &input_ids, const GenerationConfig &gen_config,
                                          int n_past, int n_ctx) -> int32_t {
...
auto QwenForCausalLM::sampling_repetition_penalty(float *first, float *last, const std::vector<int> &input_ids,
                                                  float penalty) -> void {
...
auto QwenForCausalLM::sampling_temperature(float *first, float *last, float temp) -> void {
...
auto QwenForCausalLM::sampling_top_k(TokenIdScore *first, TokenIdScore *kth, TokenIdScore *last) -> void {
    std::nth_element(first, kth, last, std::greater<TokenIdScore>());
}

auto QwenForCausalLM::sampling_top_p(TokenIdScore *first, TokenIdScore *last, float top_p) -> TokenIdScore * {
...
auto QwenForCausalLM::sampling_softmax_inplace(TokenIdScore *first, TokenIdScore *last) -> void {
...
auto QwenForCausalLM::generate(const std::vector<int> &input_ids, const GenerationConfig &gen_config,
                               BaseStreamer *streamer) -> std::vector<int> {
    ...
    while ((int)output_ids.size() < gen_config.max_length) {
        ...
        if (next_token_id == config.eos_token_id || next_token_id == config.im_start_id ||
            next_token_id == config.im_end_id || next_token_id == config.pad_token_id) {
            break;
        }
    }
...
auto Llama3ForCausalLM::load(ModelLoader &loader) -> void {
    // std::cout << "Debug: state_dict size: " << state_dict_.size() <<std::endl;
    for (auto &item : state_dict_) {
        const std::string &name = item.first;
        ggml_tensor *tensor = item.second;
...
auto QwenForCausalLM::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx, bool is_decoding) const
    -> ggml_tensor * {
...
auto QwenMoeForCausalLM::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx,
                                 bool is_decoding) const -> ggml_tensor * {
    ggml_tensor *transformer_outputs =
        transformer.forward(ctx, input_ids, n_past, n_ctx, config.num_experts, config.num_experts_per_tok);
    // NOTE: only compute next_token_logits for the last token
    if (is_decoding && input_ids->ne[0] > 1) {
        transformer_outputs = tensor_assign_buffers(
            ggml_view_1d(ctx->ctx_b.get(), transformer_outputs, config.hidden_size,
...
auto Llama3ForCausalLM::forward(ModelContext *ctx, ggml_tensor *input_ids, int n_past, int n_ctx,
                                bool is_decoding) const -> ggml_tensor * {
    ggml_tensor *transformer_outputs = transformer.forward(ctx, input_ids, n_past, n_ctx);
    // NOTE: only compute next_token_logits for the last token
    if (is_decoding && input_ids->ne[0] > 1) {
...
Pipeline::Pipeline(const std::string &path, const std::string &tiktoken_path, int max_length) {
    ...
    if (model_type == ModelType::QWEN2) {
        ...
        config = std::make_unique<Llama3Config>(loader.read_basic<Llama3Config>());
        _update_config_max_length(*config, max_length);
        Llama3Config *llama3_config = static_cast<Llama3Config *>(config.get());
        model = std::make_unique<Llama3ForCausalLM>(*llama3_config);
    }
    ...
    if (model_type == ModelType::LLAMA3) {
        tokenizer = std::make_unique<LlamaTokenizer>(tiktoken_path, *config);
    } else {
        tokenizer = std::make_unique<QwenTokenizer>(tiktoken_path, *config);
    }
}

auto Pipeline::generate(const std::vector<int> &input_ids, const GenerationConfig &gen_config,
                        BaseStreamer *streamer) const -> std::vector<int> {
...
auto Pipeline::generate(const std::string &prompt, const GenerationConfig &gen_config, BaseStreamer *streamer) const
    -> std::string {
