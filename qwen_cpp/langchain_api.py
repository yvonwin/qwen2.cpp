...
async def chat(body: ChatRequest) -> ChatResponse:
    ...
    for prompt, response in body.history:
        messages += [
            qwen_cpp.ChatMessage(role="user", content=prompt),
            qwen_cpp.ChatMessage(role="assistant", content=response),
        ]
    messages.append(qwen_cpp.ChatMessage(role="user", content=body.prompt))
